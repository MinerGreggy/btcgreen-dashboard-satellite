...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'btcgreen-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('btcgreen').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('btcgreen')[service]);
